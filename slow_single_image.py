...
def args():
    ...
    parser.add_argument('--content', help='content image', type=str, default='./image_input/c_golden_gate.jpg')
    parser.add_argument('--style', help='style image', type=str, default='./image_input/style.jpg')
    parser.add_argument('--vggfile', help='locaiton of pre-trained vgg model', type=str, default='./models/imagenet-vgg-verydeep-16.mat')
    parser.add_argument('--save_dir', help='path to save output images', type=str, default='./image_output/')
    parser.add_argument('--lr', help='learning rate', type=float, default=10)
    parser.add_argument('--max_iteration', help='total number of iterations for training ', type=int, default=500)
    parser.add_argument('--eval_iteration', help='evaluate loss and save the output image per number of iterations', type=int, default=100)
...
def main():
    ...
    # initialize the default tf graph
    with tf.Graph().as_default():
        ...
        # load pre-trained vgg neural networks
        vgg16_filters = load_vgg16(options.vggfile)
        vgg16 = Network(VGG16DESC, vgg16_filters)
        # evaluate the vgg layers of content, style and output images
        ...
        # a combination of the content loss (content vs output image on certian layers of vgg), the style loss (style vs output image on certian layers of vgg) and the total variation of output image
        c_loss = 0.0
        ...
        tv_loss = TV_WEIGHTS*(tf.nn.l2_loss(o_img[:,1:,:,:] - o_img[:,:-1,:,:]) + tf.nn.l2_loss(o_img[:,:,1:,:] - o_img[:,:,:-1,:]))
        loss = c_loss + s_loss + tv_loss
        # define the tf optimizer
        train_step = tf.train.AdamOptimizer(options.lr).minimize(loss)

        # initialize, iterate and evaluate the training
        with tf.Session() as sess:
            ...
            for i in range(options.max_iteration):
                ...
                if  (i+1) % options.eval_iteration == 0:
                    ...
                    time_start = datetime.now()
                    save_img = np.clip(o_img.eval()[0], 0, 255).astype(np.uint8)
                    save_img = Image.fromarray(save_img, "RGB")
                    save_img.save(options.save_dir+'stylized_image_{}.jpg'.format(int((i+1)/options.eval_iteration)))
            print 'Training finished.'

class Network:
    """ initiate neural network based on description string and pre-trained models, and evaluate layers given conent/style/output images
    ...
    def eval(self, i_img, layers):
        ...
        for k in range(max_layer):
            ...
            if layer_desc[0] == 'conv2d':
                o_img = tf.nn.conv2d(o_img, self.filters[layer_name][0], [1, int(layer_desc[5]), int(layer_desc[6]), 1], 'SAME' )
                o_img = tf.nn.bias_add(o_img, self.filters[layer_name][1])
            elif layer_desc[0] == 'relu':
                o_img = tf.nn.relu(o_img)
            elif layer_desc[0] == 'mpool':
                    o_img = tf.nn.max_pool(o_img, (1,int(layer_desc[1]),int(layer_desc[2]),1), (1,int(layer_desc[3]),int(layer_desc[4]),1), 'SAME')
...
def preprocess(image):
    ...
    subtract the mean (in RGB) [123.68, 116.779, 103.939]
    """
