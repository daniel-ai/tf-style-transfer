...
# description of neural network layers in the form of 'layer_type-input_channels-output_channels-kernel_height-kernel_width-stride_height-stride-width' if the parameters are needed for the corresponding layer_type
# layer_type norm = instance normalization
# layer_type res = a residual block of [conv, norm, relu, conv, norm]
# layer_type tanh_resc = rescaled (tanh+1)*255/2
# only the first 23 layers of VGG 16 are used
...
MODELDESC = 'conv2d-9-9-3-32-1-1, norm-32, relu, conv2d-3-3-32-64-2-2, norm-64, relu, conv2d-3-3-64-128-2-2, norm-128, relu, res-3-3-128-128-1-1, res-3-3-128-128-1-1, res-3-3-128-128-1-1, res-3-3-128-128-1-1, res-3-3-128-128-1-1, tconv2d-3-3-128-64-2-2, norm-64, relu, tconv2d-3-3-64-32-2-2, norm-32, relu, tconv2d-9-9-32-3-1-1, tanh_resc'
# weights used to evaluate content and style transfer loss
C_WEIGHTS = dict([('layer_16',1)])
S_WEIGHTS = dict([('layer_4',0.01), ('layer_9',0.01), ('layer_16',0.01), ('layer_23',0.01)])
TV_WEIGHTS = 0.00001

def args():
    parser = ArgumentParser()
    parser.add_argument('--name', help='name of the style transfer model', type=str, default='train')
    ...
    parser.add_argument('--style', help='the styple image to be trained', type=str, default='./image_input/style.jpg')
    parser.add_argument('--train_dir', help='directory of content images for training', type=str, default='./COCO/')
    parser.add_argument('--image_size', help='size of training images', nargs='+', type=int, default=[256, 256])
    parser.add_argument('--vggfile', help='location of trained vgg model', type=str, default='./models/imagenet-vgg-verydeep-16.mat')
    ...
    parser.add_argument('--global_step', help='global count of training iterations', type=int, default=0)
    parser.add_argument('--max_iteration', help='maximum number of iterations for training ', type=int, default=20000)
    parser.add_argument('--eval_iteration', help='evaluate loss per number of iterations', type=int, default=1000)
    parser.add_argument('--save_iteration', help='save session file per number of iterations', type=int, default=5000)
    parser.add_argument('--stylize', help='indicator whether to stylize images or train models', action='store_true')
    parser.add_argument('--model', help='model file for save/restore', type=str, default='./models/model_single')
    parser.add_argument('--output_dir', help='path to save stylized images', type=str, default='./image_output/')
    options = parser.parse_args()
    return options

def main():
    """ train sing style transfer models or stylize images
    """
    options = args()
    with tf.Graph().as_default():
        if options.stylize:
            stylize(options.name, options.content_dir, options.model, options.output_dir)
        else:
            train(options.name, options.style, options.train_dir, options.image_size, options.global_step, options.lr, options.batch_size, options.val_size, options.vggfile, options.model, options.max_iteration, options.eval_iteration, options.save_iteration)

def train(name, style, train_dir, image_size, global_step, lr, batch_size, val_size, vggfile, model, max_iteration, eval_iteration, save_iteration):
    ...
    content_image = tf.placeholder(tf.float32, input_shape, name='content_image')
    style_net = Network(MODELDESC, name, trainable=True)
    output_image = style_net.image(content_image)

    c_loss, s_loss, tv_loss = loss_vgg(style_image, content_image, output_image, vggfile)
    ...
    with tf.Session() as sess:
        ...
        train_images.index = global_step % (train_images.number - batch_size*val_size) + batch_size*val_size
        ...
        for i in range(max_iteration):
            ...
            if  (i+1) % eval_iteration == 0 or (i+1) == max_iteration:
                ...
                val_losses = np.stack([sess.run([c_loss, s_loss, tv_loss], {content_image: val_image}) for val_image in val_set])
            ...
            if  (i+1) % save_iteration == 0 or (i+1) == max_iteration:
                ...
                print 'Session file model-{} saved.'.format(global_step)
                sys.stdout.flush()
...
def stylize(name, content_dir, model, output_dir):
    ...
    with tf.Session() as sess:
        ...
        for c_name in content_images.keys():
            ...
            output_image = Image.fromarray(output_image.eval()[0].astype(np.uint8), "RGB")
            output_image.save(output_dir+'stylized_{}.jpg'.format(c_name))
    print '{} stylized image saved to {}.'.format(len(content_images), output_dir)

class train_set:
    """ training data feeder
    """
    def __init__(self, train_dir):
        self.files = [os.path.join(root,filenm) for root, dirs, files in os.walk(train_dir) for filenm in files if filenm[0:4] == 'COCO']
...
def loss_vgg(style_image, content_image, output_image, vggfile):
    ...
    tv_loss = TV_WEIGHTS*(tf.nn.l2_loss(output_image[:,1:,:,:] - output_image[:,:-1,:,:]) + tf.nn.l2_loss(output_image[:,:,1:,:] - output_image[:,:,:-1,:]))
...
class Network:
    ...
    2. to train a neural network, initialize the parameters and feed the training data to calculate losses

    """
    def __init__(self, modeldesc, name, initial=None, trainable=False, process=False):
        self.name = name
        ...
        for k, layer_desc in enumerate(self.layers_desc):
            ...
            with tf.variable_scope('{}_{}'.format(name, layer_name)):
                if layer_desc[0] == 'res':
                    self.filters[layer_name] = res_filter_init(layer_desc, layer_initial, trainable)
                else:
                    self.filters[layer_name] = basic_filter_init(layer_desc, layer_initial, trainable)

    def layers(self, image, layers):
        """ evaluate certain vgg layer values given an image
        """
        output_image = preprocess(image) if self.process else image
        eval_layers = {}
        max_layer_n = max([int(layer[6:]) for layer in layers])
        for k in range(max_layer_n):
            layer_name = 'layer_{}'.format(k+1)
            if self.layers_desc[k][0] == 'res':
                output_image = res_layer_eval(output_image, self.layers_desc[k], self.filters[layer_name])
            else:
                output_image = basic_layer_eval(output_image, self.layers_desc[k], self.filters[layer_name])
    ...
    def image(self, image):
        ...
        return self.layers(image, [last_layer_name])[last_layer_name]
...
def res_layer_eval(image, layer_desc, layer_filter):
    ...
    output_image = basic_layer_eval(output_image, conv_layer_desc, layer_filter[4:6])
...
def basic_layer_eval(image, layer_desc, layer_filter):
    ...
    if layer_desc[0] == 'conv2d':
        ...
        return tf.nn.batch_normalization(image, mean, variance, offsets, scales, 1e-5)
    elif layer_desc[0] == 'tconv2d':
        b, h, w, c = image.get_shape().as_list()
        s_1, s_2 = map(int, layer_desc[5:7])
        image = tf.nn.conv2d_transpose(image, layer_filter[0], [b,s_1*h,s_2*w,int(layer_desc[4])], [1,s_1,s_2,1], 'SAME')
        return tf.nn.bias_add(image, layer_filter[1])
...
def res_filter_init(layer_desc, layer_initial, trainable):
    ...
    with tf.variable_scope('conv_1'):
        c1_layer_initial = layer_initial[0:2] if layer_initial is not None else None
        conv_1 = basic_filter_init(conv_layer_desc, c1_layer_initial, trainable)
    with tf.variable_scope('norm_1'):
        n1_layer_initial = layer_initial[2:4] if layer_initial is not None else None
        norm_1 = basic_filter_init(norm_layer_desc, n1_layer_initial, trainable)
    with tf.variable_scope('conv_2'):
        c2_layer_initial = layer_initial[4:6] if layer_initial is not None else None
        conv_2 = basic_filter_init(conv_layer_desc, c2_layer_initial, trainable)
    with tf.variable_scope('norm_2'):
        n2_layer_initial = layer_initial[6:8] if layer_initial is not None else None
        norm_2 = basic_filter_init(norm_layer_desc, n2_layer_initial, trainable)
...
def basic_filter_init(layer_desc, layer_initial, trainable):
    ...
    if layer_desc[0] in ['relu', 'mpool', 'tanh_resc', 'sigmoid_res']:
        ...
        if not trainable and layer_initial is not None:
            ...
            if layer_desc[0] == 'conv2d':
                return [tf.get_variable('weights', map(int, layer_desc[1:5]), initializer=tf.truncated_normal_initializer(stddev=0.01)), tf.get_variable('biases', [int(layer_desc[4])], initializer=tf.constant_initializer(0))]
            elif layer_desc[0] == 'tconv2d':
                h, w, i_c, o_c = map(int, layer_desc[1:5])
                return [tf.get_variable('weights', [h, w, o_c, i_c], initializer=tf.truncated_normal_initializer(stddev=0.01)), tf.get_variable('biases', [o_c], initializer=tf.constant_initializer(0))]
            elif layer_desc[0] == 'norm':
                return [tf.get_variable('scales', int(layer_desc[1]), initializer=tf.constant_initializer(1)), tf.get_variable('offsets', int(layer_desc[1]), initializer=tf.constant_initializer(0))]
        else:
            raise ValueError("Please load trained nerual network for initialization.")

def padding(image, kernel, stride, mode='REFLECT'):
...
def load_vgg(file):
    ...
    for k in range(len(vgg_layers)):
        if vgg_layers[k][0][0][1][0] == 'conv':
            ...
            weights = np.transpose(vgg_layers[k][0][0][2][0][0], (0,1,2,3))
    ...
    for root, dirs, files in os.walk(train_dir):
        for filenm in files:
            ...
            if image.shape[0]/image.shape[1] >= image_size[0]/image_size[1]:
                ...
                image = scipy.misc.imresize(image, [image_size[0], int(image_size[1]*image.shape[1]/image.shape[0])])
